...
The interpreter should be able to infer from newview the following:
    ...
    v output (explicitly stated) overwritten.) integer-indexable collections of numbers of used_size >= num_states
    if tip_case in [PLL_TIP_INNER, PLL_INNER_INNER]
...
newviewi:
    tip_case: loop invariant switch possible in loop over n
    increased integer dimension of vl, vr, le, ri, v
